    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <!-- Payment Terms quick links (CRUD) -->
                @if(auth()->user()->is_admin || auth()->user()->is_root || auth()->user()->hasPermission('settings.view'))
                <div class="mr-4" x-data="{ openTerms: {{ request()->routeIs('payment-terms.*') ? 'true' : 'false' }} }">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                                <div class="flex items-center">
                                    <i class="fas fa-calendar-alt mr-2"></i> Payment Terms
                                </div>
                                <div class="ms-1">
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="route('payment-terms.index')">
                                {{ __('All Payment Terms') }}
                            </x-dropdown-link>
                            @if(auth()->user()->is_admin || auth()->user()->is_root || auth()->user()->hasPermission('settings.edit'))
                            <x-dropdown-link :href="route('payment-terms.create')">
                                {{ __('Create Payment Term') }}
                            </x-dropdown-link>
                            @endif
                        </x-slot>
                    </x-dropdown>
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('manual')" target="_blank" rel="noopener">
                            {{ __('User Manual') }}
                        </x-dropdown-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('manual')" target="_blank" rel="noopener">
                    {{ __('User Manual') }}
                </x-responsive-nav-link>
