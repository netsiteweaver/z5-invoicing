...
@section('content')
<div class="grid grid-cols-1 gap-6 lg:grid-cols-3" x-data="paymentForm()">
	<!-- Sales selector -->
	<div class="lg:col-span-1 bg-white shadow rounded-lg p-6">
		<h3 class="text-lg font-medium text-gray-900 mb-4">Select Sale</h3>
		@if($preselectedSale)
			@php $due = max(0, (float) $preselectedSale->total_amount - (float) $preselectedSale->payments()->where('status',1)->sum('amount')); @endphp
			<div class="p-4 border rounded-md bg-gray-50">
				...
				<div class="mt-1 text-sm font-medium text-gray-900">Amount Due: Rs {{ number_format($due, 2) }}</div>
				<input type="hidden" id="due_amount" value="{{ number_format($due, 2, '.', '') }}">
		...
		@else
			...
			<div class="mt-4 divide-y divide-gray-200 border rounded-md">
				@forelse($sales as $sale)
					@php $due = max(0, (float) $sale->total_amount - (float) $sale->payments()->where('status',1)->sum('amount')); @endphp
					<div class="p-4 flex items-center justify-between">
						<div>
							...
							<div class="text-xs text-gray-500">Due: Rs {{ number_format($due, 2) }} · Status: {{ ucfirst($sale->payment_status) }}</div>
						</div>
	...
	<div class="lg:col-span-2 bg-white shadow rounded-lg p-6">
		...
		@endif
		<form method="POST" action="{{ route('payments.store') }}" class="space-y-4" @submit.prevent="validateAndSubmit($event)">
			@csrf
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div>
					<label class="block text-sm font-medium text-gray-700">Sale</label>
					<select name="sale_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required x-ref="saleSelect">
				...
				<div>
					...
					<input type="number" step="0.01" min="0.01" name="amount" x-ref="amount" value="{{ old('amount') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="0.00" required>
					<p class="mt-1 text-xs text-gray-500">Must be less than or equal to amount due.</p>
				</div>
				<div>
			...
			<div class="pt-2">
				<x-action-button type="save">Save Payment</x-action-button>
			</div>
		</form>
	</div>
</div>
...
function paymentForm() {
	return {
		validateAndSubmit(e) {
			...
			const due = dueEl ? parseFloat(dueEl.value) : Number.POSITIVE_INFINITY;
			const amount = parseFloat(this.$refs.amount.value || '0');
			if (amount > due) {
				alert(`Amount exceeds amount due (Rs ${due.toFixed(2)}).`);
				return false;
			}
			e.target.submit();
		}
