                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <div class="flex flex-col items-end space-y-1">
                            @if($sale->status === 'draft')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{{ ucfirst($sale->status) }}</span>
                            @elseif($sale->status === 'confirmed')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{{ ucfirst($sale->status) }}</span>
                            @elseif($sale->status === 'shipped')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">{{ ucfirst($sale->status) }}</span>
                            @elseif($sale->status === 'delivered')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">{{ ucfirst($sale->status) }}</span>
                            @elseif($sale->status === 'cancelled')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">{{ ucfirst($sale->status) }}</span>
                            @endif
                            
                            @if($sale->payment_status === 'pending')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">{{ ucfirst($sale->payment_status) }}</span>
                            @elseif($sale->payment_status === 'partial')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{{ ucfirst($sale->payment_status) }}</span>
                            @elseif($sale->payment_status === 'paid')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">{{ ucfirst($sale->payment_status) }}</span>
                            @elseif($sale->payment_status === 'overdue')
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">{{ ucfirst($sale->payment_status) }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-3">
                        <div>
                            ...
                            <div class="text-sm font-medium text-gray-900">{{ $sale->sale_date ? \Carbon\Carbon::parse($sale->sale_date)->format('M d, Y') : 'N/A' }}</div>
                            @if($sale->due_date)
                    ...
                    <div class="text-xs text-gray-500 mb-3">{{ $sale->items->count() }} items</div>
                    
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('sales.show', $sale) }}" class="inline-flex items-center px-3 py-2 bg-blue-600 text-white text-xs font-medium rounded-md hover:bg-blue-700 transition-colors">
                            <i class="fas fa-eye mr-1"></i> View
                        </a>
                        @if($sale->canBeEdited())
                            <a href="{{ route('sales.edit', $sale) }}" class="inline-flex items-center px-3 py-2 bg-gray-600 text-white text-xs font-medium rounded-md hover:bg-gray-700 transition-colors">
                                <i class="fas fa-pen mr-1"></i> Edit
                            </a>
                        @endif
                        @if($sale->payment_status !== 'paid')
                            <a href="{{ route('sales.payments.create', $sale) }}" class="inline-flex items-center px-3 py-2 bg-green-600 text-white text-xs font-medium rounded-md hover:bg-green-700 transition-colors">
